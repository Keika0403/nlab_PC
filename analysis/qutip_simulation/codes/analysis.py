...
class SechPhotonAnalysis:
    ...
    def analyze_waveforms(self, passing_band, legend=False, savefig=False, mode_opt=False, target_waveform=[]):
        ...
        for i, (k, v) in enumerate(result_items):
            ...
            ax_t.plot(time, res, "r", lw=2.0, alpha=0.7, label="Target")
            ax_t.plot(time, waveform, "tab:blue", alpha=0.3, lw=0.5, label="Data")
            ax_t.plot(time, sign*signal.real, "tab:blue", lw=1.0, linestyle="-", label="Real")
            ax_t.plot(time, sign*signal.imag, "tab:blue", lw=1.0, linestyle="--", label="Imag")
            ax_t.set_xlim(0, self.duration)
            ax_t.set_ylim(-0.15, 0.15)
            ax_t.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
            if i == nrows - 1:
                ax_t.set_xlabel("Time (ns)")
            ...
            if mode_id % 2 == 0:
                sign = np.sign(np.vdot(fourier.real, fourier_tar.real))
                fourier *= sign
                ax_f.plot(photon_freq_tar, fourier_tar.real, "r", lw=2.0, alpha=0.7, label="Target")
                ax_f.plot(photon_freq, fourier.real, "tab:blue", lw=1.0, linestyle="-", label="Real")
                ax_f.plot(photon_freq, fourier.imag, "tab:blue", lw=1.0, linestyle="--", label="Imag")
            else:
                sign = np.sign(np.vdot(fourier.imag, fourier_tar.imag))
                fourier *= sign
                ax_f.plot(photon_freq_tar, fourier_tar.imag, "r", lw=2.0, alpha=0.7, label="Target")
                ax_f.plot(photon_freq, fourier.real, "tab:blue", lw=1.0, linestyle="-", label="Real")
                ax_f.plot(photon_freq, fourier.imag, "tab:blue", lw=1.0, linestyle="--", label="Imag")
...
class SpatiotemporalAnalysis:
    ...
    def plot_results(self, ax=None, label=None, color=None, fitting=True, init_fit=False, err_bar=False, x_target=None):
        if ax is None:
            fig, ax = plt.subplots()
    ...
    def plot_waveform(self, ax=None, title=None, ph_amp_idx=None, fogi_freq_idx=None, show_legend=False, show_title=False, color="tab:blue", mode=0):
        if ax is None:
            fig, ax = plt.subplots()
        ...
        if mode%2==0:
            ...
            sign = -1
        ax.plot(time_data, sign*y_reflect, lw=2.5, alpha=0.4, color=color, label="w/o fogi")
        ax.plot(time_data, sign*y_absorb, lw=1.5, color=color, linestyle="--",label="w/ fogi")
        ax.set_ylim(-0.7, 0.9)
        ax.set_xlim(0, 1400)
