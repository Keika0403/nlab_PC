...
class WaveformAnalysisMulti:
    ...
    def gamma_t(self, time, const, plot=0, form="sech_0", nc_id=0):
        ...
        if form == "nc":
            ...
            sech = np.array([sech_ac(ti, nc_id, const*2) for ti in t-center])
            top = np.abs(sech)**2
            bottom = 1 - cumulative_trapezoid(top, t-center, initial=0)
            bottom[bottom <= 1e-12] = np.nan
    ...
    def generate_control_pulse(self, fogi_lo, target_freq, duration, kappa, form="sech", mode_id=0, plot=True):
        ...
        ## polynominal
        res_poly = polynomial_even_fit(x=amps, data=Gammas)
        c = tuple(res_poly.params.valuesdict().values())
        x = np.linspace(0, amps[-1], 10001)
        Gamma_to_amp_poly = interpolate.interp1d(polynomial_even(x, c[0], c[1], c[2], c[3], c[4], c[5]),
                                             x, bounds_error=False, fill_value=(0, amps[-1]))
        if form == "sech":
            target = sech_m(time, mode_id, kappa)
        AWGamp_of_time_poly = lambda time:Gamma_to_amp_poly(self.gamma_t(time, np.array(target)))

        
        envelope_poly = AWGamp_of_time_poly(time)
        zero_points_poly = self.find_zero_point(envelope_poly)
        flipped_envelope_poly = self.flip_control_envelope(envelope_poly, zero_points_poly)

        ## spline
        spline_fit = CubicSpline(amps, Gammas, bc_type='natural')
        gamma_dense = spline_fit(x)
        Gamma_to_amp_spline = interpolate.interp1d(gamma_dense, x, bounds_error=False, fill_value=(0, amps[-1]))
        AWGamp_spline = lambda t: Gamma_to_amp_spline(self.gamma_t(t, const=const, form=form, nc_id=nc_id))

        envelope_spline = AWGamp_spline(time)
        zero_points_spline = self.find_zero_point(envelope_spline)
        flipped_envelope_spline = self.flip_control_envelope(envelope_spline, zero_points_spline)

        if plot:
            fig = plt.figure(figsize = (6.5, 3))
            ax = fig.add_subplot(1, 2, 1)
            ax.plot(amps, res_poly.best_fit * 1e3,'r-', label='fit_poly')
            ax.plot(x, spline_fit(x)*1e3,'g-', label='fit_spline')
            ax.plot(amps, np.array(Gammas)*1e3,'ko')
            ax.set_xlabel('Drive amplitude (V)')
            ax.set_ylabel('$\Gamma_{\mathrm{f}}$ (MHz)')
            ax.legend()
            virtual_Gamma = np.linspace(0, 20e-3, 10001)
            ax2 = fig.add_subplot(1, 2, 2)
            ax2.plot(virtual_Gamma, Gamma_to_amp_poly(virtual_Gamma),'r-', label='fit_poly')
            ax2.plot(virtual_Gamma, Gamma_to_amp_spline(virtual_Gamma),'g-', label='fit_spline')
            ...
            ax = fig.add_subplot(1, 2, 1)
            ax2 = fig.add_subplot(1, 2, 2)
            ax.plot(time, flipped_envelope_poly)
            ax.set_title('AWGamplitude vs time')
            ax2.plot(time, np.array([sech_ac(ti, nc_id, const*2) for ti in time-duration/2]))
        ...
        def fogi_freq_of_time(time):
            f_freq = fogi_freq_of_AWGamp(AWGamp_of_time_poly(time))
            return f_freq
        ...
        waveform_at_AWG_poly = flipped_envelope_poly*np.exp(2j * np.pi * phase_of_time)
        waveform_at_AWG_spline = flipped_envelope_spline*np.exp(2j * np.pi * phase_of_time) 
        if plot:
            plt.figure(figsize=(4, 3))
            plt.plot(time,waveform_at_AWG_poly.real)
            plt.plot(time,waveform_at_AWG_poly.imag)
            plt.plot(time,flipped_envelope_poly)
            plt.plot(time,flipped_envelope_spline)
            plt.show()
        return waveform_at_AWG_poly, flipped_envelope_poly, waveform_at_AWG_spline, flipped_envelope_spline
...
class SechPhotonAnalysis:
    ...
    def fit_waveform(self, time, data, x0=[2.5e-3, 450], get_center=False, curve='sech_0'):
    ...
    def analyze_waveforms(self, passing_band, flipped=False):
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            ax.plot(photon_freq, 10 * np.abs(fourier), label=f"data")
            ...
            zero_points = self.find_zero_point(envelope=np.abs(signal))
            flipped_env = self.flip_control_envelope(np.abs(signal), zero_points)

            res = self.fit_waveform(time, np.abs(signal), curve=f"{form}")
            zero_points = self.find_zero_point(envelope=res)
            flipped_envelope = self.flip_control_envelope(res, zero_points)

            ax2.plot(time, 1000 * waveform, "tab:blue", lw=1, alpha=0.5)
            if flipped:
                ax2.plot(time, 1000 * flipped_env, "tab:blue", lw=3, label=f"data")
                ax2.plot(time, 1000 * flipped_envelope, "r--", lw=2, label="target", alpha=1)
            else:
                ax2.plot(time, 1000 * np.abs(signal), "tab:blue", lw=3, label=f"data")
    ...
    def analyze_waveforms_t(self, passing_band, flipped=False):
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            res = self.fit_waveform(time, np.abs(signal),x0=[2.5e-3, v["center"]], curve=f"{form}")
            zero_points = self.find_zero_point(envelope=res)
            flipped_envelope = self.flip_control_envelope(res, zero_points)

            ax.plot(time, 2000 * waveform, "tab:blue", lw=0.5, alpha=0.5)
            if flipped:
                # ax.plot(time, 1000 * flipped_env, "tab:blue", lw=3)
                ax.plot(time, 2000 * flipped_envelope, "r--", lw=2)
            else:
                # ax.plot(time, 1000 * np.abs(signal), "tab:blue", lw=3)
                ax.plot(time, 2000 * res, "r--", lw=2)
        ...
        handles = [
            plt.Line2D([], [], color="tab:blue", lw=3, label="data"),
            plt.Line2D([], [], color="r", lw=2, linestyle="--", label="target")
        ]
...
class SpatiotemporalAnalysis:
    ...
    def _process_ctrl_pulse(self, datadict, times, signals):
        ...
        # norm=np.sum(2*np.abs(waveform_complex)**2)**0.5
        # signals.append(waveform_complex/norm)
        # plt.plot(waveform_complex)
        # plt.plot(signal)
        # plt.show()
    ...
    def overlap_comm(self, data_tx, data_rx, taus):
        ...
        for tau in taus:
            ...
            # plt.plot(data_tx.imag)
            # plt.plot(np.conjugate(shifted_data_rx).imag)
            # plt.show()
            # print(shifted_data_rx)
    ...
    def plot_results(self, ax=None, label=None, color=None, fitting=True):
        ...
        ax.plot(np.ravel(self.results['fogi_delay']), self.rates, 
                "o", label=label, color=color)
        ax.errorbar(np.ravel(self.results['fogi_delay']), self.rates, 
            yerr=self.results['rates_err'], fmt='o', 
        ...
        if fitting:
            ...
            ax.plot(np.ravel(self.results['fogi_delay']), fit_result.best_fit, "-", label=label, color=color)
            # ax.plot(np.ravel(self.results['fogi_delay']), fit_result.init_fit, "-", label=label, color=color)
        return ax
    ...
    def plot_absorption_rate_matrix(data):
        ...
        plt.imshow(data, extent=extent, filternorm=False, vmin=0, vmax=100)
        ...
        for j in range(num_y_ticks):
            for k in range(num_x_ticks):
                ...
                plt.text(x_ticks[j], 3 - y_ticks[k], f"{value}", ha='center', va='center', color="red", fontsize=20)
