...
class WaveformAnalysisMulti:
    ...
    def gamma_t(self, time, const, plot=0, form="sech",):
        ...
        if form=="sech":
            ...
            # bottom = 1-(1/2)*(np.tanh(const*(time-center))+np.tanh(const*center))
            ...
            top = squared_anti_sech(time-center)
            bottom = []
            for t in time:
                bottom = np.append(bottom, 1- quad(squared_anti_sech, center, t)[0]** 2)
        if plot:
    ...
    def generate_control_pulse(self, fogi_lo, target_freq, duration, const, form="sech", plot=True):
        ...
        def fogi_freq_of_time(time):
            return fogi_freq_of_AWGamp(AWGamp_of_time(time))
