...
class WaveformAnalysisMulti:
    ...
    def generate_results(self, lo_freq):
        for k, datadict in self.datadicts.items():
            ...
            original_decay_rates = self.results_dict[k]["decay_rates"]
    ...
    def plot_decayrates(self):
        ...
        for k, v in self.results_dict.items():
            ax.plot(v["fogi_freqs"], v["decay_rates"], '.-',label=f"{self.amps_dict[k]}")
            # ax2.plot(v["photon_freqs"], v["decay_rates"], '.-',label=f"amp {self.amps_dict[k]}")
        ax.set_xlabel('F0g1 frequency $\omega$ (GHz)')
        ax.set_ylabel('$\Gamma_f $ /2$\pi$ (GHz)')
        # ax2.set_xlabel('photon freq (GHz)')
        # ax2.set_ylabel('$\Gamma_f $ (MHz)')
        ax.legend(title= r"F0g1 amplitude O (V)", bbox_to_anchor=(1.6, 1.1), loc='upper right')
        ax.grid()
        # ax2.grid()
        plt.tight_layout()
        plt.show()
