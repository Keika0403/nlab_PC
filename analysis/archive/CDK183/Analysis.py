    ...
    def extract_decay_rate_and_freq(self, fit_start=30, fp=0.02, fs=0.05, gpass=1, gstop=90, plot=False, skip=[]):
        ...
        for i, waveform in enumerate(self.waveforms):
            ...
            if plot:
                ...
                axes[i].legend()
                axes[i].set_title(f"Freq: {photon_frequency[np.argmax(np.abs(fourier))]}")
                axes[i].set_xlabel("Time")
                axes[i].set_ylabel("Amplitude")

                axes_freq[i].plot(photon_frequency, np.abs(fourier), label="|FFT|", lw=2)
                axes_freq[i].set_title(f"Freq: {photon_frequency[np.argmax(np.abs(fourier))]}")
...
class SechPhotonAnalysis:
    ...
    def fit_waveform(self, time, data, kappa, x0=[2.5e-3, 450], form='sech', mode_id=0, target_waveform=[]):
    ...
    def analyze_waveforms(self, passing_band, legend=False, savefig=False, mode_opt=False, target_waveform=[]):
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            res = self.fit_waveform(time, signal, self.kappa, x0, form, mode_id, target_waveform)
            sign = np.sign(np.vdot(res, signal.real))
            print("sign:", sign)
            signals.append(signal*sign)
            ax_t.plot(time, 2000 * res, "r", lw=2.5,  alpha=0.7, label="Target")
            ax_t.plot(time, 2000*waveform, "tab:blue", alpha=0.3, lw=0.5, label="Data")
            ax_t.plot(time, 2000*sign*signal.real, "tab:blue", lw=1.2,linestyle="-", label="Real") 
            ax_t.plot(time, 2000*sign*signal.imag, "tab:blue", lw=1.2,linestyle=(0, (2, 1)),  label="Imag")
            ax_t.label_outer()
            ...
            xfft_tar_no_pad, fourier_tar_no_pad = fourier_no_padding(time, res * np.exp(-2j * np.pi * (self.readout_lo_freq - target_freq) * time))
            ...
            ax_f.plot(photon_freq_tar, 10 * np.abs(fourier_tar)/2,  "r", lw=2.5, alpha=0.7, label=f"data")
            ax_f.plot(photon_freq, 10 * np.abs(fourier), "tab:blue", lw=1.2, linestyle="-", label=f"data")
    ...
    def optimize_modes(self, target_freq, photon_freq, freq_axes, fouriers, fourier_tars, transmissivity, passing_band):
        ...
        fig_trans, ax = plt.subplots(figsize=(4, 2.5))
...
class SpatiotemporalAnalysis:
    ...
    def plot_absorption_rate_matrix(data, ax):
        ...
        for j in range(num_y_ticks):
            for k in range(num_x_ticks):
                value = r"$\sim 0$" if data[k, j] < 0 else f"{data[k, j]:.2f}"
                if data[k, j] > 0.5:
    ...
    def plot_waveform(self, ax=None, title=None, ph_amp_idx=None, fogi_freq_idx=None, show_legend=False):
        if ax is None:
            fig, ax = plt.subplots()
        ...
        ax.plot(time_data, y_reflect, lw=3, color='tab:blue', label="w/o fogi")
        ax.plot(time_data, y_absorb, lw=3, color='tab:red', label="w/ fogi")
