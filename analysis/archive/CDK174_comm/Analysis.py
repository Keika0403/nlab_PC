...
class SechPhotonAnalysis:
    ...
    def sech_curve(x, time, kappa, order):
        t_shift = time - x[1]
        if order == 0:
            return np.abs(x[0] / np.cosh(kappa * t_shift))
        elif order == 1:
    ...
    def analyze_waveforms(self, passing_band, flipped=False):
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            res = self.fit_waveform(time, np.abs(lpf), curve=f"{form}")
            ...
            if flipped:
                ax2.plot(time, 1000 * lpf, "tab:blue", lw=3, label=f"data")
    ...
    def analyze_waveforms_t(self, passing_band, flipped=False, dc=0):
        fig, axes = plt.subplots(2, 2, figsize=(3.2, 1.8), sharex=True, sharey=True)
        # fig, axes = plt.subplots(2, 2, figsize=(5.2, 2.7), sharex=True, sharey=True)
        axes = axes.flatten()
        signals = []
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            phase = np.angle(demodulate(time, waveform, self.readout_lo_freq - target_freq))
            # phase = np.angle(waveform * np.exp(1j * 2 * np.pi * (self.readout_lo_freq - target_freq) * time))
            lpf = 2 * lowpass(
                time, waveform * np.exp(2j * np.pi * (self.readout_lo_freq - target_freq) * time),
                passing_band, 0.03, 0.1, 90
            ) 
            signal =lpf* np.exp(-1j * phase)
            # global_phase =- 3*np.pi/8
            # signal =signal* np.exp(-1j * global_phase)
            signals.append(signal)
            res = self.fit_waveform(time, np.abs(signal),x0=[2.5e-3, v["center"]], curve=f"{form}")
            zero_points = self.find_zero_point(envelope=res)
            flipped_envelope = self.flip_control_envelope(res, zero_points)
            waveform = waveform *np.exp(-1j*2*np.pi*dc*time)
            # ax.plot(time, 2000 * waveform, "tab:blue", lw=1, alpha=0.5, label="data")
            # ax.plot(time, 2000 * lpf, "tab:blue", lw=1, alpha=1)

            if flipped:
                # ax.plot(time, 1000 * flipped_env, "tab:blue", lw=3)
                ax.plot(time, 2000 * flipped_envelope, "r", lw=2,  alpha=0.7, label="target")
            else:
                # ax.plot(time, 1000 * np.abs(signal), "tab:blue", lw=3)
                ax.plot(time, 2000 * res, "r", lw=2,  alpha=0.7, label="target")

            # ax.plot(time, np.angle(signal), color="gray", linestyle=":", label="phase")

            ax.plot(time, 2000*waveform, "tab:blue", alpha=0.3, lw=0.5, label="waveform")
            # ax.plot(time, 2000*np.abs(signal), "g--",  label="envelope")
            ax.plot(time, 2000*signal.real, "tab:blue", lw=1.5,linestyle="-", label="real")
            ax.plot(time, 2000*signal.imag, "tab:blue", lw=1.5,linestyle=(0, (2, 1)),  label="imag")
        ...
        # fig.legend(by_label.values(), by_label.keys(), loc='upper right', ncol=2, bbox_to_anchor=(0.5, 1.07), frameon=False)
        fig.text(0.04, 0.6, "Amplitude (arb. unit)", va="center", rotation="vertical")
        self.waveform_envelope = signals
        handles = [
            plt.Line2D([], [], color="tab:blue", lw=3, label="data"),
            plt.Line2D([], [], color="r", lw=2,  label="target")
        ]
    ...
    def analyze_waveforms_f(self, passing_band):
        fig, axes = plt.subplots(2, 2, figsize=(3.2, 1.8), sharex=True, sharey=True)
        # fig, axes = plt.subplots(2, 2, figsize=(5.2, 2.7), sharex=True, sharey=True)
        axes = axes.flatten()
        signals = []
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            zero_points = self.find_zero_point(envelope=res)
            flipped_envelope = self.flip_control_envelope(res, zero_points)
            xfft_tar, fourier_tar = fourier_tr_padding(time,  flipped_envelope* np.exp(-2j * np.pi * (self.readout_lo_freq - target_freq)* time))
            xfft_tar, fourier_tar = start_stop(xfft_tar, fourier_tar, xfft_tar[0], 0)
            photon_freq_tar = self.readout_lo_freq + xfft_tar

            ax.plot(photon_freq_tar, 10 * np.abs(fourier_tar)/2,  "r", lw=2, alpha=0.7, label=f"data")
            ax.plot(photon_freq, 10 * np.abs(fourier), "tab:blue", lw=1.5, linestyle="-", label=f"data")
            # ax.vlines(target_freq, 0, 10 * max(np.abs(fourier)) * 1.1, color="r", ls="--", label="target")
            ax.set_xlim(9.33, 9.37)
    ...
    def plot_squaredI_matrix(self, mode_num):
        ...
        for m in range(mode_num):
            for m_prime in range(mode_num):
                wf_m = self.waveform_envelope[m]
                wf_m_prime = self.waveform_envelope[m_prime]
        ...
        return I_normalized, self.waveform_envelope
...
class SpatiotemporalAnalysis:
    ...
    def process_data(self):
        for p in range(self.num_of_ph_amp):
            ...
            for n in range(len(self.fogi_delay[0])):
                ...
                y_absorbed_LPF = LPF(y_absorbed_shift, 500e6, 20e6, 30e6, 5, 40) * 2
                ...
                y_base_LPF = LPF(y_base_shift, 500e6, 20e6, 30e6, 5, 40) * 2
                y_base_env.append(y_base_LPF)
                
                E = np.sum(np.abs(y_absorbed_LPF) ** 2) * 2
                E1 = np.sum(np.abs(y_base_LPF) ** 2) * 2
    ...
    def overlap_comm(self, data_tx, data_rx, taus):
        ys = []
        # x = np.arange(0, 1002, 2)
        # data_tx = LPF(data_tx * np.exp(-1j * 2 * np.pi * self.f_if * (x * 1e-9)), 500e6, 20e6, 30e6, 5, 40) * 2
        norm=np.sum(2*np.abs(data_tx)**2)**0.5
        data_tx =-data_tx/norm
        # plt.plot(data_tx)
        # plt.plot(data_rx)
        # plt.show()
    ...
    def plot_results(self, ax=None, label=None, color=None, fitting=True, init_fit=False, err_bar=False):
        if ax is None:
            fig, ax = plt.subplots()
    ...
    def plot_absorption_rate_matrix(data, ax):
        ...
        for j in range(num_y_ticks):
            for k in range(num_x_ticks):
                value = r"$\sim 0$" if int(np.round(data[k, j])) < 0 else f"{data[k, j]:.2f}"
                if data[k, j] > 0.5:
                    ax.text(x_ticks[j], 3 - y_ticks[k], value, ha='center', va='center', color="red", fontsize=10)
                else:
                    ax.text(x_ticks[j], 3 - y_ticks[k], value, ha='center', va='center', color="white", fontsize=10)
