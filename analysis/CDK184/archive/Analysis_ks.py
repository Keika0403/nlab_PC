...
class WaveformAnalysisMulti:
    ...
    def generate_control_pulse(self, fogi_lo, target_freq, duration, kappa, form="sech", mode_id=0, plot=True):
        ...
        if plot:
            ...
            plt.plot(time,envelope.real)
            plt.plot(time,envelope.imag)
            plt.show()
...
class SechPhotonAnalysis:
    ...
    def fit_waveform(self, time, data, kappa, x0=[2.5e-3, 450], form='sech', mode_id=0):
        ...
        sign = np.sign(np.vdot(fitted, data.real))
        print("sign:", sign)
        return sign * fitted
    ...
    def analyze_waveforms(self, passing_band):
        ...
        fig_t, axes_t = plt.subplots(2, 2, figsize=(3.2, 1.8), sharex=True, sharey=True)
        fig_f, axes_f = plt.subplots(2, 2, figsize=(3.2, 1.8), sharex=True, sharey=True)
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            res = self.fit_waveform(time, signal, self.kappa, x0, form, mode_id)
            
            ax_t.plot(time, 2000*waveform, "tab:blue", alpha=0.3, lw=0.5, label="waveform")
            ax_t.plot(time, 2000*signal.real, "tab:blue", lw=1.5,linestyle="-", label="real")
            ax_t.plot(time, 2000*signal.imag, "tab:blue", lw=1.5,linestyle=(0, (2, 1)),  label="imag")
            ax_t.plot(time, 2000 * res, "r", lw=2,  alpha=0.7, label="target")
            ax_t.label_outer()
            ax_t.set_xlim(0, self.duration)
            if i in [2, 3]:
                ax_t.set_xlabel("Time (ns)")

            xfft_tar, fourier_tar = fourier_tr_padding(time,  res* np.exp(-2j * np.pi * (self.readout_lo_freq - target_freq)* time))
            xfft_tar, fourier_tar = start_stop(xfft_tar, fourier_tar, xfft_tar[0], 0)
            photon_freq_tar = self.readout_lo_freq + xfft_tar

            ax_f.plot(photon_freq_tar, 10 * np.abs(fourier_tar)/2,  "r", lw=2, alpha=0.7, label=f"data")
            ax_f.plot(photon_freq, 10 * np.abs(fourier), "tab:blue", lw=1.5, linestyle="-", label=f"data")
            ax_f.set_xlim(target_freq-0.02, target_freq+0.02)
            ax_f.label_outer()
            if i in [2, 3]:
                ax_f.set_xlabel("Frequency (GHz)")
        fig_t.text(0.0, 0.6, "Amplitude (arb. unit)", va="center", rotation="vertical")
        fig_f.text(0.0, 0.6, "Amplitude (arb. unit)", va="center", rotation="vertical")
        self.times = times
    ...
    def phase_subtraction(self, ctrl_pulse_path, save=False):
        ...
        for i, (k, v) in enumerate(self.result_dict.items()):
            ...
            target_signal = self.fit_waveform(time, signal, self.kappa, x0, form, mode_id)
    ...
    def generate_control_pulse_rx(self, ctrl_pulse_path, save=False):
        ...
        for i, (key, params) in enumerate(self.result_dict.items()):
            ...
            ax.plot(np.arange(len(recieve_pulse)), recieve_pulse.real, label=f"ctrl_pulse_rx")
            ax.plot(np.arange(len(recieve_pulse)), env_rvs, label=f"ctrl_envelope_rx")
            ax.legend()
    ...
    def generate_control_pulse_rx_new(self, ctrl_pulse_path, save=False):
        fig, axes = plt.subplots(len(self.result_dict), 1, figsize=(4, 2 * len(self.result_dict)))
        ...
        for i, (key, params) in enumerate(self.result_dict.items()):
            ...
            ctrl_pulse = datadict["control_pulse"]["values"]
            env = datadict["control_envelope"]["values"]
            phase = np.unwrap(np.angle(ctrl_pulse/env))
            env_rvs = np.conj(np.flip(env))

            phase_rvs = np.flip(phase)
            inst_omega_minus = -np.diff(phase_rvs)
            phase_recieve = np.zeros(phase.shape)
            # phase_recieve[0] = phase[-1]
            for i in range(1, len(phase_recieve)):
                phase_recieve[i] = phase_recieve[i-1] + inst_omega_minus[i-1]
