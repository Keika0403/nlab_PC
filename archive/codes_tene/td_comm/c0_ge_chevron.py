...
sequence.add(Square(amplitude=1.0, duration=duration), qubit_drive_port_tx)
...
with DDH5Writer(data, data_path, name=measurement_name) as writer:
    ...
    for update_command in tqdm(variables.update_command_list):
        ...
        # sequence.draw()
        # raise SystemError
        for f in tqdm(np.linspace(7.98, 7.99, 201), leave=False):  # Hz6.85
            qubit_drive_port_tx.if_freq = f - qubit_lo_freq_tx
