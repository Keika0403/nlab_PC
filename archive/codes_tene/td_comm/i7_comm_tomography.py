...
_, datadict = search_datadict_miyamura("D:\\miyamura\\result\\control_pulses", 
                                        "2024-08-12", acquire_time="120157")
control_pulse_tx = datadict["control_pulse"]["values"].ravel()

_, datadict = search_datadict_miyamura("D:\\miyamura\\result\\control_pulses", 
                                        "2024-08-12", acquire_time="120556")
...
def exp(prepare_seq:Sequence, state:str):
    def comm_sequence(delay_relative:int,) -> Sequence:
        if delay_relative > 0 or ge_pi_pulse_tx.params["duration"] + ef_pi_pulse_tx.params["duration"] + delay_relative >= 0:
            delay = delay_relative + ge_pi_pulse_tx.params["duration"] + ef_pi_pulse_tx.params["duration"]
            assert delay%2 == 0
        ...
        elif ge_pi_pulse_tx.params["duration"] + ef_pi_pulse_tx.params["duration"] + delay_relative < 0:
            delay = -ge_pi_pulse_tx.params["duration"] - ef_pi_pulse_tx.params["duration"] - delay_relative
    ...
    def seq_qst(delay_relative:int, POVM:str)->Sequence:
        ...
        sequence.call(reset_sequence_tx)
        sequence.call(reset_sequence_rx)
        sequence.trigger(ports)
        sequence.call(readout_ss_seq_tx)
        sequence.call(readout_ss_seq_rx)
        sequence.trigger(ports)
        sequence.call(comm_sequence(delay_relative))
        sequence.trigger(ports)
        if POVM == "g":
    ...
    try:
        with DDH5Writer(data, data_dir, name=state + "_" + measurement_name) as writer:
            writer.add_tag(tags)
            ...
            for i, p in tqdm(enumerate(["g", "+ge", "ige", "e", "+gf", "igf", "f", "+ef", "ief"])):
                ...
                result_gg_tx, result_gg_rx, _, other_rx = postselection_both_sides(
                    result_tx, result_rx, ss_start_tx, ss_end_tx, ss_start_rx, ss_end_rx
                )
                # print(windows_rx)
                # print(result_rx.shape, result_gg_rx.shape)
                delete_idx = []
                for i in range(len(result_gg_tx)):
                    single_shot = result_gg_tx[i][ss2_start_tx//2:ss2_end_tx//2]
                    # single_shot_e = result_e[i][0:len(mean_e_rx)]
                    if np.dot(single_shot - mean_g_tx, ein_vec_tx) > 1/2: # eliminate e state
                        delete_idx.append(i)
                    # if np.dot(single_shot_e - mean_g_rx, ein_vec_rx) > 1/2: # eliminate e state
                    #     delete_idx_e.append(i)
                result_gg_e_rx = result_gg_rx[delete_idx]
                result_gg_e_rx = result_gg_e_rx[:, int(ss2_start_rx//2):int(ss2_end_rx//2)]
                result_gg_rx = np.delete(result_gg_rx, delete_idx, axis=0)[:, int(ss2_start_rx//2):int(ss2_end_rx//2)]
                ...
                signal_gg_e = np.dot(result_gg_e_rx - mean_g_rx, ein_vec_rx)
                signal_dsc = np.append(signal_e, signal_gg_e)
                print(f"emitted : {signal_g.shape}, not emitted {signal_dsc.shape}",)
                writer.add_data(
                    label=i, 
                    signal=np.array(list(signal_g)+list(np.inf*(np.ones(signal_dsc.shape))))
                )
    finally:
        # off()
        print('finished')
