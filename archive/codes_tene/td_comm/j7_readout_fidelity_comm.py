...
sequence_g = Sequence(port_list=[qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_g.call(readout_single_shot_seq)
sequence_g.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_g.add(Delay(30), qubit_drive_port_rx)
sequence_g.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_g.call(readout_single_shot_seq)
# sequence_g.draw()

sequence_e = Sequence(port_list=[qubit_drive_port_rx, readout_port, JPA_port, dig_port])
...
try:
    with DDH5Writer(data, data_path, name=measurement_name) as writer:
        ...
        for _ in tqdm(range(repetition)):
            ...
            post_selection_start_g = int(dig_port.measurement_windows[1][0] // 2 )
            ...
            post_selection_start_e = int(dig_port.measurement_windows[1][0] // 2 )
        ...
        delete_idx_g = []
        delete_idx_e = []
        # post_selection_start_g = int(sequence_g.get_waveform_information()[dig_port.name]['measurement_windows'][1][0] // 2 )
        # post_selection_start_e = int(sequence_e.get_waveform_information()[dig_port.name]['measurement_windows'][1][0] // 2 )
        for i in range(len(result_g)):
            ...
            if np.dot(single_shot_e - mean_g, ein_vec) > 1/2: # eliminate e state
