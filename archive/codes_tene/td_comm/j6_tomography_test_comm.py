...
sequence_x = Sequence(port_list=[qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_x.call(readout_single_shot_seq)
sequence_x.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_x.call(ge_half_pi_seq_rx)
sequence_x.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_x.add(Delay(34), qubit_drive_port_rx)
sequence_x.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_x.add(VirtualZ(0), qubit_drive_port_rx, copy=False)
sequence_x.call(ge_half_pi_seq_rx)
sequence_x.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_x.call(readout_single_shot_seq)

sequence_y = Sequence(port_list=[qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_y.call(readout_single_shot_seq)
sequence_y.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_y.call(ge_half_pi_seq_rx)
sequence_y.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_y.add(Delay(34), qubit_drive_port_rx)
sequence_y.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_y.add(VirtualZ(-np.pi/2), qubit_drive_port_rx, copy=False)
sequence_y.call(ge_half_pi_seq_rx)
sequence_y.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_y.call(readout_single_shot_seq)

sequence_z = Sequence(port_list=[qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_z.call(readout_single_shot_seq)
sequence_z.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_z.call(ge_half_pi_seq_rx)
sequence_z.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_z.add(Delay(34), qubit_drive_port_rx)
sequence_z.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_z.add(Delay(30), qubit_drive_port_rx)
sequence_z.trigger([qubit_drive_port_rx, readout_port, JPA_port, dig_port])
sequence_z.call(readout_single_shot_seq)
...
try:
    with DDH5Writer(data, data_path, name=measurement_name) as writer:
        ...
        for _ in tqdm(range(repetition)):
            load_sequence_comm(sequence_x, cycles=shot_count)
            # plt.plot(JPA_port.waveform.real)
            plt.plot(readout_port.waveform.real)
            pulse_x = run_comm(sequence_x, plot=0) * voltage_step_comm
            result_x.append(pulse_x)
            post_selection_start_x = int(dig_port.measurement_windows[1][0] // 2 )
            
            load_sequence_comm(sequence_y, cycles=shot_count)
            pulse_y = run_comm(sequence_y, plot=0) * voltage_step_comm
            result_y.append(pulse_y)
            post_selection_start_y = int(dig_port.measurement_windows[1][0] // 2 )

            load_sequence_comm(sequence_z, cycles=shot_count)
            # plt.plot(JPA_port.waveform.real)
            # plt.plot(readout_port.waveform.real)
            # plt.show()
            pulse_z = run_comm(sequence_z, plot=0) * voltage_step_comm
