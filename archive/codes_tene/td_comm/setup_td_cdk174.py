...
# tx qubit drive
lo2:N51x1 = N51x1('lo2', 'TCPIP0::192.168.100.49::inst0::INSTR')
lo2.frequency(qubit_lo_freq_tx*1e9)
lo2.power(17)
station.add_component(lo2)

# tx fogi drive
lo3:N51x1 = N51x1('lo3', 'TCPIP0::192.168.100.93::inst0::INSTR')
lo3.frequency(fogi_lo_freq_tx*1e9) 
lo3.power(16)
station.add_component(lo3)

# rx qubit drive
lo4:N51x1 = N51x1('lo4', 'TCPIP0::192.168.100.9::inst0::INSTR')
lo4.frequency(qubit_lo_freq_rx*1e9)
lo4.power(13)
station.add_component(lo4)

# rx fogi drive
lo5:E8247 = E8247('lo5', 'TCPIP0::192.168.100.7::inst0::INSTR')
lo5.frequency(fogi_lo_freq_rx*1e9) 
lo5.power(13)
station.add_component(lo5)

# rx readout
lo6:E8247 = E8247('lo6', 'TCPIP0::192.168.100.240::inst0::INSTR')
...
if yoko.state():
    ...
    print('yoko was off')
assert yoko.state()
yoko.ramp_current(99e-6, step=1e-8, delay=0)  # photon 0.65
...
yoko_tx.ramp_current(113e-6, step=1e-8, delay=0) # tx readout, current99, amp: 1.24

yoko_rx:GS200 = GS200("yoko_rx", "TCPIP0::192.168.100.193::inst0::INSTR")
# station.add_component(yoko_rx)
if yoko_rx.state():
...
yoko_rx.ramp_current(100e-6, step=1e-8, delay=0)
...
iq_corrector_fogi_tx = IQCorrector(
    ...
    awg_fogi_Q_tx,
    data_path,
    lo_leakage_datetime="2025-02-16T171052",
    rf_power_datetime="2025-02-16T171939",
    len_kernel=41,
    fit_weight=10,
)
...
iq_corrector_q_tx = IQCorrector(
    ...
    awg_Qdrive_Q_tx,
    data_path,
    lo_leakage_datetime="2025-02-16T161553",
    rf_power_datetime="2025-02-16T162419",
    len_kernel=41,
    fit_weight=10,
)
...
iq_corrector_fogi_rx = IQCorrector(
    ...
    awg_fogi_Q_rx,
    data_path,
    lo_leakage_datetime="2025-02-17T150115",
    rf_power_datetime="2025-02-17T150915",
    len_kernel=41,
    fit_weight=10,
)
...
iq_corrector_q_rx = IQCorrector(
    ...
    awg_Qdrive_Q_rx,
    data_path,
    lo_leakage_datetime="2025-02-17T141754",
    rf_power_datetime="2025-02-17T142901",
    len_kernel=41,
    fit_weight=10,
)
...
iq_corrector_JPA_tx = IQCorrector(
    ...
    awg_JPA_Q_tx,
    data_path,
    lo_leakage_datetime="2025-02-16T163711", #lo 9.47
    rf_power_datetime="2025-02-16T164456", #lo 9.47
    # lo_leakage_datetime="2025-02-22T102809", 
    # rf_power_datetime="2025-02-22T103545",
    len_kernel=41,
    fit_weight=10,
)
...
iq_corrector_JPA_rx = IQCorrector(
    ...
    awg_JPA_Q_rx,
    data_path,
    lo_leakage_datetime="2025-02-22T105221",
    rf_power_datetime="2025-02-22T110142",
    len_kernel=41,
    fit_weight=10,
)
...
def run(sequence:Sequence, plot=False, lo_on=True, which=measure_which):
    ...
    if qubit_drive_port_tx in sequence.port_list:
        ...
        if fogi_port_tx in sequence.port_list:
            ...
            awg_fogi_Q_tx.start()
    if qubit_drive_port_rx in sequence.port_list:
        assert dig_ch_tx.cycles()==awg_Qdrive_I_rx.cycles()[-1], f'{dig_ch_tx.cycles()}, {awg_Qdrive_I_rx.cycles()}'
        # assert dig_ch_rx.cycles()==awg_Qdrive_I_rx.cycles()[-1], f'{dig_ch_rx.cycles()}, {awg_Qdrive_I_rx.cycles()}'
...
def postselection_both_sides(data_tx:np.ndarray, data_rx:np.ndarray, start_tx:int|float, stop_tx:int|float, start_rx:int|float, stop_rx:int|float, which:str="both", which_result="both"):
