...
if measure_which == "tx":
    ef_pi_pulse_drag_tx.params['beta']=beta
    seq = Sequence(port_list=ports_tx)
    seq.call(ge_pi_seq_tx)
    seq.add(SetDetuning(anharmonicity_tx), qubit_drive_port_tx)
    for _ in range(num_of_pairs):
        seq.add(HalfDRAG(ef_pi_pulse_tx, beta=beta), qubit_drive_port_tx)
    ...
    ef_pi_pulse_drag_rx.params['beta']=beta
    seq = Sequence(port_list=ports_txQrx)
    seq.call(ge_pi_seq_rx)
    seq.add(SetDetuning(anharmonicity_rx), qubit_drive_port_rx)
    for _ in range(num_of_pairs):
        seq.add(HalfDRAG(ef_pi_pulse_rx, beta=beta), qubit_drive_port_rx)
    ...
    with DDH5Writer(data, data_path, name=measurement_name) as writer:
        ...
        for update_command in tqdm(var.update_command_list):
            ...
            writer.add_data(
                ...
                s11=demodulate(run(seq, which=measure_which).mean(axis=0) * voltage_step_tx),
            )
finally:
    off()
    print('finished')