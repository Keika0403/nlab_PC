...
seq_e.add(Delay(100), readout_port)
seq_e.trigger(ports_wo_JPA)
seq_e.compile()
readout_e = readout_port.waveform.real
# seq_e.trigger(ports)
seq_e.add(Square(amplitude=0.5, duration=drive_length+160), JPA_port, copy=False)
...
# # seq_e.update_variables(var.update_command_list[2])
# seq_e.compile()
# i_qubit_e, q_qubit_e = iq_corrector_q.correct(qubit_drive_port.waveform.conj())
# i_jpa_e, q_jpa_e = iq_corrector_JPA.correct(np.append(jpa_e, jpa_pulse).conj())
# plt.plot(np.append(readout_e, waveform_AWG_coherent_state_e))
# plt.plot(i_jpa_e)
# plt.plot(i_qubit_e)
# plt.show()
# seq_e.draw()
...
experiment_name = "JPA gain when qubit state = e"
        ...
        for update_command in tqdm(var.update_command_list):
            ...
            for mode in ["plus_I", "plus_Q", "minus_I", "minus_Q"]:
                ...
                for _ in range(repetition):
                    ...
                    # plt.plot(mean_e)
                    # plt.show()
                    data = postselection(data, 0, 40, state="e")
                    data = data[:, int((measurement_windows_e[1][0]-measurement_windows_e[0][0]) // dig_ch.sampling_interval()):
                ...
                print(box.shape, mode, "e")
                if mode=="plus_I": plus_I_e = box.mean(axis=0)
                if mode=="plus_Q": plus_Q_e = box.mean(axis=0)
                if mode=="minus_I": minus_I_e =  box.mean(axis=0)
                if mode=="minus_Q": minus_Q_e =  box.mean(axis=0)
            
            waveform_e = ((plus_I_e - minus_I_e) / 2 + (plus_Q_e - minus_Q_e) / 2) / 2
            datasaver.add_result(
                (amplitude_param, seq_JPA.variable_dict["amplitude"][0].value),
                (time_param, np.arange(len(waveform_e))*dig_ch.sampling_interval()), 
