...
if measure_which == "tx":
    ...
    ge_half_pi_pulse_drag_rx.params['beta']=beta
    seq = Sequence(port_list=ports_txQrx)
    for _ in range(num_of_pairs):
        seq.call(ge_half_pi_seq_rx)
        seq.add(VirtualZ(pi), qubit_drive_port_rx)
        seq.call(ge_half_pi_seq_rx)
        seq.add(VirtualZ(pi), qubit_drive_port_rx)
    ...
    seq.add(Acquire(100), dig_port_rx)
