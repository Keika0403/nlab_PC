...
# seq = Sequence(port_list = [qubit_drive_port_tx]) 
# seq.add(Square(amplitude=1, duration=2000), qubit_drive_port_tx)
# seq.compile()
# waveform_i , waveform_q = iq_corrector_q_tx.correct(qubit_drive_port_tx.waveform)
# awg2.load_waveform(waveform_i, 0, append_zeros=True)
# awg2.load_waveform(waveform_q, 1, append_zeros=True)
# awg_Qdrive_I_tx.queue_waveform(0, trigger="software/hvi", cycles=0, per_cycle=False)
# awg_Qdrive_Q_tx.queue_waveform(1, trigger="software/hvi", cycles=0, per_cycle=False)
# awg2.start_all()


seq1 = Sequence(port_list = [readout_port_rx]) 
seq1.add(Square(amplitude=1, duration=10000), readout_port_rx)
seq1.compile()
awg1.load_waveform(readout_port_rx.waveform.real, 0, append_zeros=True)
awg_readout_rx.queue_waveform(0, trigger="software/hvi", cycles=0, per_cycle=False)
awg1.start_all()
