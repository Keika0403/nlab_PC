...
##### fogi pulse
pulse_dict = dict(
    # data0 = dict(target_freq=10.3775, date="2024-07-24", acquire_time="153613"),
    # data1 = dict(target_freq=9.35, date="2024-07-31", acquire_time="005215"),
    # data2 = dict(target_freq=10.3975, date="2024-07-24", acquire_time="153616"),
    # data2 = dict(target_freq=10.3825, date="2024-07-26", acquire_time="092919"),

    #antisech
    # data0 = dict(target_freq=10.3775, date="2024-07-25", acquire_time="085411"),
    data1 = dict(target_freq=9.35, date="2024-07-31", acquire_time="172926"),
    # data2 = dict(target_freq=10.3975, date="2024-07-25", acquire_time="085415"),
)
for k, v in tqdm(pulse_dict.items()):
    ...
    fogi_duration = 1000
    fogi_delay = np.linspace(-100, 200, 76)#76)#61

    ##### coherent pusle
    ph_if = readout_lo_freq*1e9 - v["target_freq"]*1e9
    ph_amp = 0.011#0.015#0.00025#id=0
    const = 2.5e-3 * 2 * np.pi

    port_list = [fogi_port, readout_port]
    acquisition_time = fogi_duration + 1000
    num_of_cycles = 50000
    repetition = 20
    ...
    try:
        with DDH5Writer(data, data_path, name=measurement_name) as writer:
            ...
            for t in tqdm(fogi_delay):
                ...
                # ph_waveform =1/np.cosh(const * (x - fogi_duration/2))*np.cos(2*np.pi* ph_if*x*1e-9) #(x - fogi_duration/2)/np.cosh(const * (x - fogi_duration/2))*np.cos(2*np.pi* ph_if*x*1e-9) #
                y_env0 = 1/np.cosh(const * (x - fogi_duration/2)) 
                y_env1 = (x - fogi_duration/2)/np.cosh(const * (x - fogi_duration/2))
                m = sum(np.abs(y_env0))/sum(np.abs(y_env1))
                ph_waveform = (y_env0 + y_env1*m)*np.cos(2*np.pi* ph_if*x*1e-9)
                ...
                def abs_sequence(fogi:bool, photon:bool, JPA_direction=1) -> Sequence:
                    ...
                    seq.trigger(ports)
                    seq.add(Square(amplitude=JPA_direction*0.8, duration=len(control_pulse_f)+1000), JPA_port)
