...
wiring = "\n".join([
    ...
    "RFout_fogi1 - 3dB - 2dB - (ZX60-83LN-S+) - 1000mm - BPF - DC(through) - In1D",
...
tags = ["TD", "CDY166", "BW9500_R"]
data_path = f"D:\\K_Sunada\\result\\{tags[1]}"

# Frequency settings
readout_freq = 9.35
readout_lo_freq = 9.5#9.054 
readout_if_freq = readout_lo_freq - readout_freq
JPA_if_freq = readout_freq * 2 - readout_lo_freq * 2

qubit_lo_freq = 7.5
ge_freq = 7.7 - 0.0259 + 6.4e-6 + 0.001075
anharmonicity = 7.315 - ge_freq -0.007955 - 0.00073 + 0.0008629 + 8.09e-6 -0.00067037 + 9.9e-6
...
readout_pulse = Square(amplitude=1.0, duration=400)
...
# ge pi pulse
ge_pi_pulse = Gaussian(amplitude= 0.9827, fwhm=10, duration=20, zero_end=True)
ge_pi_pulse_drag = HalfDRAG(ge_pi_pulse, beta= 0.712)#1.83)
ge_pi_seq = Sequence()
...
ge_half_pi_pulse_drag = HalfDRAG(ge_half_pi_pulse, beta= 0.755)#1.83)#0.310)#-1.032) # 1.35
ge_half_pi_seq = Sequence()
ge_half_pi_seq.add(ge_half_pi_pulse_drag, qubit_drive_port, copy=False)


# ef pi pulse
ef_pi_pulse = Gaussian(amplitude=0.5667, fwhm=10, duration=20, zero_end=True)
ef_pi_pulse_drag = HalfDRAG(ef_pi_pulse, beta= -0.7852)##0.154)#0.9223)
...
ef_half_pi_pulse_drag = HalfDRAG(ef_half_pi_pulse, beta=-0.8185)#0.228)#1.027)
...
# # Reset pulse
Appeared_anhm = SetDetuning(-0.3725)
reset_pulse_ef = FlatTop(Gaussian(amplitude=0.00563, fwhm=12, duration=40), top_duration=1000)
reset_pulse_fogi = FlatTop(Gaussian(amplitude=0.5, fwhm=12, duration=40), top_duration=1000)
...
reset_sequence.add(SetDetuning((5.6-fogi_lo_freq) - fogi_port.if_freq), fogi_port, copy=False)
# reset_sequence.add(SetDetuning(fogi_if_freq - fogi_port.if_freq), fogi_port, copy=False)
